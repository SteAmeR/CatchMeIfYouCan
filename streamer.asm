...
start:
; dummy code
; Çogu APT analiz ürünü Runtime esnasýnda analiz ettikleri sample'in adını değiştirerek işe başlarlar. 
; Böylece her sample için atanan unique ve analiz süresince aynı kalan dosya adı sayesinde 
; farkli analiz aşamalarında ki bilgi değiş tokuşu sağlanmış olur. 
...
; Aslinda yaptigi asagidakilerden ibarettir.
	...
	invoke GetCurrentProcess ; eax ezildi yukarýda bu yüzden stack'e push ettik
	mov ecx, esp  
	mov edx, [esp] ; NtQueryInformationProcess'in adresini eax le stacke atmıştık şimdi geri alıyoruz
	invoke PNTQUERYINFORMATIONPROCESS ptr edx,eax,7,addr dwDebugPort,4,NULL ;7 = ProcessDebugPort
	pop edx
	cmp byte ptr [dwDebugPort], 0 ; deðer sıfır dışında bir şey ise debug ediliyoruz.
	jne debugger_found_4_2

; phase_5
; Processinizi biri debugger ile açmýþ ve üstüne birde step step kodları analiz ediyorsa bunu anlamak
; için aşağıda ki kod bloğunu devreye sokuyoruz. 
